...
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ ⚠️ 阶段四：提交修订并更新状态 【强制执行区 - 不可跳过】               │
├─────────────────────────────────────────────────────────────────┤
│ 1. 给工单添加评论说明修订内容                                      │
│ ⚠️ 2. 移除工单标签 `status/revising`   【强制执行】                 │
│ ⚠️ 3. 添加工单标签 `status/reviewing`  【强制执行】                 │
└─────────────────────────────────────────────────────────────────┘
```
...
> ⚠️ **【强制执行区】** 此阶段包含 **2 个严格按顺序执行的标签更新步骤（4.2-4.3）**，必须在评论添加完成后执行！
>
> **执行顺序不可调换：必须先移除 `status/revising`，再添加 `status/reviewing`**
